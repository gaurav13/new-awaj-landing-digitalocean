...
/**
 * One-time (repeatable, idempotent) backfill: for every application that was submitted before
 * People/Organization auto-sync existed, ensure the central Organization and Person records
 * exist and are linked. Approved applications produce public (published/approved) records;
 * pending/info-requested applications produce non-public (draft/pending) records. Rejected
 * applications are skipped. Safe to run multiple times — existing records are reused, not duplicated.
 */
  ...
  try {
    ...
    for (const app of allApps) {
      ...
      if (personName?.trim()) {
        ...
        peopleCount++
        // Ensure the person is linked and their visibility matches the application status.
        await db
          .update(people)
    ...
    console.error("[member-applications] sync failed:", err)
